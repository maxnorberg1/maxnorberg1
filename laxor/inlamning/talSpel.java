...
public class talSpel {
		...
		while (!repeat) {
			...
			while (!success) {
				...
				try {
					...
					if (firstChoice < 1 || firstChoice > 4) { // Kontrollerar att spelarens input är inom intervallet
																// 1-4.
	...
	public static void game() {
		...
		do { // Låter spelaren välja svårhetsgrad, och kontrollerar att inputen är inom de
				// givna siffrorna.
			diff = in.nextInt();
			if (diff > 3 || diff < 0) {
				System.out.println("** You did not enter a integer. Try again. **");
		...
		if (diff == 1) { // Spelet på lättaste nivån, 15 gissningar.
			System.out.println("Easy difficulty\n");
			for (int i = 14; i >= 0; i--) { // Låter spelaren gissa tills han/hon gissar rätt eller får slut på
											// gissningar.
				...
				if (i == 0) { // Skriver ut ett meddelande till spelaren då han/hon har fått slut på
								// gissningar.
		...
		} else if (diff == 2) { // Spelet på nivå Medium, 10 gissningar.
			System.out.println("Medium difficulty\n");

			for (int i = 9; i >= 0; i--) { // Låter spelaren gissa tills han/hon gissar rätt eller får slut på
											// gissningar.
		...
		} else if (diff == 3) { // Spelet på den svåraste nivån, 5 gissningar.
			System.out.println("Hard difficulty\n");

			for (int i = 4; i >= 0; i--) { // Låter spelaren gissa tills han/hon gissar rätt eller får slut på
											// gissningar.
	...
	/**
	 * Metod som används för att fråga spelaren om han/hon vill spela igen.
	 * Innehåller en trycatch för att spelet inte ska krascha vid felaktig input.
	 */
	public static void playAgain() {
		...
		while (!success) {
			...
			try {
				...
				if (playAgain == 0) { // Avgör om spelet ska spelas igen / gå tillbaka till menyn / avsluta spelet.
					System.out.println("Game is closed.");
					System.exit(0);
				} else if (playAgain == 1) {
					game();
	...
	/**
	 * Metod som används då användaren ska gå tillbaka till menyn. Innehåller en try
	 * catch för att spelet inte ska krascha vid felaktig input.
	 */
